...
function DisqusJS(config) {
    ((window, document, localStorage, fetch, Promise) => {
        ...
        */
        ...
        const HTML_TPL_EL_ASK_FOR_FULL = '欲参与讨论，请对 disq.us | disquscdn.com | disqus.com 启用代理并 <a id="dsqjs-reload-disqus" class="dsqjs-msg-btn"></a><a id="dsqjs-force-disqus" class="dsqjs-msg-btn">加载 Disqus</a>';
        ...
        function loadDisqus() {
            if (window.DISQUS) {
                ...
                $$(DISQUS_CONTAINER_EL_ID).innerHTML = `<div id="dsqjs"><section><div id="dsqjs-msg">评论加载中... 如果长时间无法加载，请针对 disq.us | disquscdn.com | disqus.com 启用代理并 <a id="dsqjs-force-dsqjs" class="dsqjs-msg-btn">加载评论基础模式</a></div></section>${HTML_TPL_EL_FOOTER}</div>`
        ...
        function loadDsqjs() {
             ...
             */
            const url = `${disqusjs.config.api}3.0/threads/list.json?forum=${encodeURIComponent(disqusjs.config.shortname)}&thread=${encodeURIComponent(`link:http://gaoryrt.com${document.location.pathname}`)}&api_key=${encodeURIComponent(apikey())}`;
  
            _get(url)
                .then(({ data }) => new Promise((res, rej) => {
                  const data1 = data
                  const post1 = data.response[0].posts
                  console.log(data)
                  if (data.code !== 0 || data.response.length !== 1 || post1 === 0) {
                      _get(`${disqusjs.config.api}3.0/threads/list.json?forum=${encodeURIComponent(disqusjs.config.shortname)}&thread=${encodeURIComponent(`link:http://gaoryrt.com${document.location.pathname}index.html`)}&api_key=${encodeURIComponent(apikey())}`).then(({data}) => {
                        const data2 = data
                        if (data.code === 0 && data.response.length === 1 && data.response[0].posts > post1) {
                          res(({data:data2}))
                        } else {
                          res(({data:data1}))
                        }
                ...
                if (data.code === 0 && data.response.length === 1) {
                    const { id, title, isClosed, posts: length, identifiers, slug } = data.response[0];
                    disqusjs.page = { id, title, isClosed, length, comment: [], slug };
                    disqusjs.config.identifier = identifiers[0]
                    // 在 #disqus_thread 中填充 DisqusJS Container
                    $$(DISQUS_CONTAINER_EL_ID).innerHTML = `<div id="dsqjs">${HTML_TPL_EL_HEADER(length, disqusjs.config.siteName)}<section class="dsqjs-post-container"><ul class="dsqjs-post-list" id="dsqjs-post-container"><p class="dsqjs-no-comment">评论列表加载中...</p></ul><a id="dsqjs-load-more" class="dsqjs-load-more dsqjs-hide">加载更多评论</a></section><div id="dsqjs-msg">评论加载中... ${HTML_TPL_EL_ASK_FOR_FULL}</div>${HTML_TPL_EL_FOOTER}</div>`;
                    ...
                    msg('当前 Thread 尚未创建。是否<a id="dsqjs-force-disqus" class="dsqjs-msg-btn">强制使用 Disqus</a>？')
                    $$('dsqjs-force-disqus').addEventListener(CLICK, useDisqus);
                } else {
                    throw new Error;
                }
            ...
            const renderComment = (data) => {
                ...
                const childrenComments = (data) => {
                    ...
                    children.map((comment) => {
                        ...
                        const hasMoreEl = comment.hasMore
                            ? `<p class="dsqjs-has-more">切换至 <a class="dsqjs-has-more-btn" id="load-more-${comment.comment.id}" data-more-id="comment-${comment.comment.id}"> Disqus 模式</a> 显示更多回复</p>`
                            : '';
                ...
                parseCommentData(data).map((comment) => {
                    ...
                    let hasMoreEl = '';
                    if (comment.hasMore) {
                        hasMoreEl = `<p class="dsqjs-has-more">切换至 <a id="load-more-${comment.comment.id}"> Disqus 模式</a> 显示更多回复</p>`
                    }
        ...
        function loadError(err) {
            console.log(err);
            msg('评论加载失败，是否 <a id="dsqjs-reload-dsqjs" class="dsqjs-msg-btn">重试</a> 或 <a id="dsqjs-reload-disqus" class="dsqjs-msg-btn">尝试加载 Disqus</a> ？');
        ...
        window.disqus_config = function () {
            this.page.identifier = disqusjs.page.id
            this.page.slug = disqusjs.page.slug
            this.page.url = disqusjs.config.url;
            this.page.identifier = disqusjs.config.identifier;
            this.page.title = disqusjs.config.title;
        };
